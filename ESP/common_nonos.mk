...
#CFLAGS		= -Og -g -O1 -std=gnu90 -Wpointer-arith -Wundef -Werror -Wl,-EL -fno-inline-functions -nostdlib -mlongcalls -mtext-section-literals -mno-serialize-volatile -D__ets__ -DICACHE_FLASH
...
CFLAGS += -DSPI_FLASH_SIZE_MAP=$(size_map)
CFLAGS += -D$(SDK_VER)
...
$1/%.o: %.c
	$(vecho) "CC $$<"
	$(Q) $(CC) $(INCDIR) $(MODULE_INCDIR) $(EXTRA_INCDIR) $(SDK_INCDIR) $(CFLAGS) -D_APP_=$(app) -D_TRG_=$(trg) -c $$< -o $$@
$1/%.o: %.cpp
	$(vecho) "C+ $$<"
	$(Q) $(CXX) $(INCDIR) $(MODULE_INCDIR) $(EXTRA_INCDIR) $(SDK_INCDIR) $(CXXFLAGS) -D_APP_=$(app) -D_TRG_=$(trg) -c $$< -o $$@
endef

.PHONY: all checkdirs clean flash flashboot flashinit rebuild flashssl
...
$(TARGET_OUT): $(APP_AR)
	...
	@echo Mode: $(SPI_MODE)/$(mode) Frq: $(SPI_SPEED)/$(freqdiv) Size: $(SPI_SIZE_MAP)/$(size_map) App: $(APP)/$(app) Boot: $(boot)
	$(Q)$(MEMANALYZER) $@
...
#		$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x00000 $(SDK_BASE)/bin/boot_v1.3.bin
#		$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x00000 $(SDK_BASE)/bin/$(BOOTLDR)
		$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x00000 bootloader/$(BOOTLDR)
            endif
        endif
    endif
endif
...
flashssl:
	$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x77000 cssl/esp_ca_cert.bin 0x78000 cssl/esp_cert_private_key.bin
...
ifeq ($(boot), none)
	...
	$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) read_mac
endif
endif
...
flashinit:
	$(vecho) "Flash init data default and blank data." $(flashimageoptions)
	$(ESPTOOL) -p $(ESPPORT) erase_flash
	$(ESPTOOL) -p $(ESPPORT) write_flash $(flashimageoptions) $(sys) $(SDK_BASE)/bin/esp_init_data_default_v05.bin
#	$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x00000 $(SDK_BASE)/bin/$(BOOTLDR)
	$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x00000 bootloader/$(BOOTLDR)
	$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x77000 cssl/esp_ca_cert.bin 0x78000 cssl/esp_cert_private_key.bin
...
flashblank:
	...
	$(ESPTOOL) -p $(ESPPORT) write_flash $(flashimageoptions) 0x7e000 $(SDK_BASE)/bin/blank.bin 0x3fc000 $(SDK_BASE)/bin/esp_init_data_default.bin
	$(ESPTOOL) -p $(ESPPORT) -b $(ESPBAUD) write_flash $(flashimageoptions) 0x77000 cssl/esp_ca_cert.bin 0x78000 cssl/esp_cert_private_key.bin
	$(ESPTOOL) -p $(ESPPORT) write_flash $(flashimageoptions) 0x79000 $(SDK_BASE)/bin/blank.bin 0x7A000 $(SDK_BASE)/bin/blank.bin 0x7B000 $(SDK_BASE)/bin/blank.bin 
