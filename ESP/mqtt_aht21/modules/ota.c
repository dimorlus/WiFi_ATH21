...
LOCAL void ICACHE_FLASH_ATTR ota_connect_cb(void *arg)
{
...
LOCAL void ICACHE_FLASH_ATTR ota_dns_found(const char *name, ip_addr_t *ipaddr, void *arg)
{
  struct espconn *pespconn = (struct espconn *)arg;
  ota_t *ota_client = (ota_t *)pespconn->reverse;
...
LOCAL void ICACHE_FLASH_ATTR start_ota_cb(ota_t *ota_client)
{
  // connection
...
void ICACHE_FLASH_ATTR start_ota(char *url)
{
  ...
  if(system_upgrade_userbin_check() == UPGRADE_FW_BIN1) str[i-1] = '2'; // /api/sensor1/user2.bin
  if(system_upgrade_userbin_check() == UPGRADE_FW_BIN2) str[i-1] = '1'; // /api/sensor1/user1.bin
